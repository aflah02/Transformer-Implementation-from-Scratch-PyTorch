...
class SelfAttention(nn.Module):
    def __init__(self, embed_size, heads):
        ...
        self.fc_out = nn.Linear(heads*self.embed_size, embed_size)
...
class DecoderBlock(nn.Module):
    def __init__(self, embed_size, heads, forward_expansion, dropout, device):
        ...
        self.transformer_block = TransformerBlock(embed_size=embed_size, dropout=dropout, forward_expansion=forward_expansion)
        self.dropout = nn.Dropout(dropout)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, enc_out, src_mask, trg_mask):
        N, seq_length = x.shape
        positions = torch.arange(0, seq_length).expans(N, seq_length).to(self.device)
